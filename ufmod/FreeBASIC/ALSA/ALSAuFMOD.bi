...
'                    param is the image size. Once, uFMOD_ALSAPlaySong
...
'    szPCM_dev
'       ASCII identifier of the ALSA PCM device. Check the ALSA
'       documentation for a complete reference of valid PCM device
'       names. "plughw:0,0" should work fine for most applications.
'  ---
'  Return Values:
'  ---
'     On success, returns an ALSA output device handle. Returns 0 on
'     failure. If you are familiar with ALSA, you'll know what this handle
'     might be useful for :)
'  ---
'  Remarks:
'  ---
'     If no valid song is specified and there is one currently being
'     played, uFMOD_ALSAPlaySong just stops playback.
'     Once playback has started, it's not necessary to check for buffer
'     underruns, since uFMOD performs buffer recovering automatically.
declare function uFMOD_ALSAPlaySong alias "uFMOD_ALSAPlaySong" (byval as any ptr, byval as uinteger, byval as uinteger, byval as zstring ptr) as uinteger
#define uFMOD_StopSong() uFMOD_ALSAPlaySong(0, 0, 0, 0)
...
'     uFMOD_ALSAPlaySong calls. You can set the desired volume level before
