...
static int ufmodplay(void* clientdata, void* interp, int argc, char *argv[]){
	if(argc != 3 || !uFMOD_ALSAPlaySong(argv[1], 0, XM_FILE, argv[2]))
...
int Ufmod_Init(void *interp){
	...
	Tcl_CreateCommand(interp, "uFMOD_ALSAPlaySong", ufmodplay,     0, 0);
	...
	Tcl_CreateExitHandler(ufmodstop, 0);
	return 0;
}
